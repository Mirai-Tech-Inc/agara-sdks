...
export class PublicClient extends Transport {
   ...
   *
   * @remarks
   * Callable without credentials. Use canonical security/provider identifiers from discovery; the
   * requested second and actual observation time are distinct.
   *
   * @param query - Canonical symbol/provider identifiers and the requested at timestamp in Unix
   * seconds.
   * @param options - Per-request abort signal, timeout override and successful-response observer.
   ...
   *
   * @remarks
   * Callable without credentials. Preserve exact provider price representations; missing
   * observations do not imply a zero price.
   *
   * @param query - Canonical symbol/provider and inclusive from/to Unix seconds over a window of at
   * most 24 hours.
   * @param options - Per-request abort signal, timeout override and successful-response observer.
...
export class TraderClient extends PublicClient {
  ...
  /**
   * Read provider-issued deposit addresses for your trading wallet.
   *
   * @remarks
   * Requires a personal access token with scope `portfolio:read`. Use the address family for the
   * selected source chain. Retrieving an address does not submit a deposit transfer.
   *
   * @param options - Per-request abort signal, timeout override and successful-response observer.
  ...
  /**
   * List supported source chains and tokens for bridge deposits.
   *
   * @remarks
   * Requires a personal access token with scope `portfolio:read`. Provider identifiers are not
   * necessarily EVM addresses or numeric chain IDs. Amounts for quotes use the source token native
   * base units.
   *
   * @param options - Per-request abort signal, timeout override and successful-response observer.
   * @returns Provider chain/token identifiers, token decimals, address family and minimum checkout
   * values.
   * @throws TypeError for invalid local request shapes or text inputs.
   * @throws RangeError for invalid local numeric inputs or timeout overrides.
   * @throws AgaraError for an unsuccessful HTTP response; inspect its status and validated
  ...
  /**
   * Estimate a cross-chain deposit without executing a transfer.
   *
   * @remarks
   * Requires a personal access token with scope `portfolio:read`. A quote is an estimate and can
   * change. Its ID is not a transaction hash or execution confirmation.
   *
   * @param body - Provider source-chain and token identifiers plus a positive integer string in the
   * source token base units.
   * @param options - Per-request abort signal, timeout override and successful-response observer.
   * @returns Estimated destination quantity, timing, USD amounts, fees and provider quote identity.
   * @throws TypeError for invalid local request shapes or text inputs.
   * @throws RangeError for invalid local numeric inputs or timeout overrides.
   * @throws AgaraError for an unsuccessful HTTP response; inspect its status and validated
   * recovery.
  ...
  /**
   * Read the latest completed settled-PnL snapshot.
   *
   * @remarks
   * Requires a personal access token with scope `portfolio:read`. Availability depends on
   * deployment configuration and completed projections. Missing or not-ready data is not a zero-PnL
   * result.
   *
   * @param options - Per-request abort signal, timeout override and successful-response observer.
  ...
  /**
   * Read completed settled-PnL buckets and range boundary snapshots.
   *
   * @remarks
   * Requires a personal access token with scope `portfolio:read`. Preserve has_more: a truncated
   * result is not a complete range. A missing boundary precedes available completed projections.
   *
   * @param query - Inclusive RFC 3339 from/to instants and optional completed-bucket limit (default
   * 672, maximum 2880).
   * @param options - Per-request abort signal, timeout override and successful-response observer.
